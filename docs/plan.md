...
3. 严格模式下禁用 `any`，并尽可能提高代码质量要求。
4. 为减少一次性引入过多阻塞，严格模式优先采用 `warning` 提示，而不是额外新增大量 `error`。
5. 发布产物、包导出、测试命令和文档说明都要能覆盖新入口。
...
2. `package.json`
   ...
   - 在 `exports` 中新增 `./strict`。
   - 新增严格模式测试脚本，例如 `test:strict`。
3. `tsdown.config.ts`
   - 将 `strict.js` 加入构建入口列表，确保生成 `dist/strict.cjs`。
4. `README.md`
   - 补充严格模式的安装与使用示例。
   - 解释默认模式与严格模式的区别。
5. 测试目录
...
1. 先直接在 `index.js` 与 `strict.js` 间通过 `extends` 复用。
2. 若后续发现 TypeScript 规则段重复过多，再考虑把共享规则抽到独立内部模块。
...
2. 新建 `test/strict.test.ts`，包含以下样例：
   ...
   - 不安全成员访问
   - 不安全返回值
3. 保留现有 `test:ts`，验证默认入口不因剥离严格规则而失效。
4. 运行 `npm run build`，确认 `dist/strict.cjs` 正常生成。

### 验收标准

1. `eslint-config-cmyr` 默认入口在普通 TypeScript 项目中不再强依赖高强度类型检查能力。
2. `eslint-config-cmyr/strict` 可独立启用，并对 `any` 发出告警。
3. 严格模式新增规则以 `warning` 为主，不产生大量新的阻塞型 `error`。
4. `package.json`、构建配置、README、测试脚本均已覆盖严格模式入口。

## 实施步骤建议

1. 调整 `index.js`，移除默认入口中的类型感知严格层。
2. 新建 `strict.js`，基于默认入口叠加严格规则。
3. 更新 `package.json` 与 `tsdown.config.ts`，补齐发布和构建入口。
4. 增加严格模式测试样例与测试脚本。
5. 更新 `README.md` 和 `CHANGELOG.md`。
6. 执行默认模式与严格模式的 lint、构建验证。
...
2. `strict.js` 承载高标准 TypeScript 规则，并以 `warning` 为主逐步收紧。
3. 同步补齐导出、构建、测试和文档，确保严格模式成为正式可发布能力。